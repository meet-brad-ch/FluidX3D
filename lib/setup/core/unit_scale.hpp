...
public:
    ...
    float pressure(Pressure p) const;                   ///< a pressure difference (the lattice pressure is (rho-1)/3)
    std::uint64_t time_steps(Duration t) const;         ///< rounded to the nearest step
    /// @}
